#!/usr/bin/env bash
# post-commit hook: Codex wiki gap detection + maintenance
...
if [ -x "$SCRIPTS_DIR/codex-wiki-gap-detection.sh" ]; then
  nohup "$SCRIPTS_DIR/codex-wiki-gap-detection.sh" > /dev/null 2>&1 &
fi
