...
# Wiki Maintenance (Codex)
#
# Detects stale wiki fragment links and uses Codex to update prose,
...
LOG_FILE="$REPO_ROOT/$LOG_DIR/codex-wiki-maintenance.log"
...
echo "Starting Codex Wiki Maintenance..."
...
if ! (cd "$WORKTREE_DIR" && timeout 900 codex --approval-mode full-auto "$PROMPT"); then
  echo "Codex exited with error — wiki maintenance aborted" >&2
  exit 1
fi
...
WORKTREE_WIKI_FILES=$(cd "$WORKTREE_DIR" && git diff --name-only HEAD \
  ...
  | grep -v 'codex-commit-message\.txt' || true)
