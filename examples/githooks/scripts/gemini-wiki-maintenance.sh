...
# Wiki Maintenance (Gemini)
#
# Detects stale wiki fragment links and uses Gemini to update prose,
...
LOG_FILE="$REPO_ROOT/$LOG_DIR/gemini-wiki-maintenance.log"
...
echo "Starting Gemini Wiki Maintenance..."
...
Write to \`$COMMIT_MSG_FILE\` using write_file. Format:
...
" | tee "$GEMINI_OUTPUT"); then
  echo "Gemini exited with error — wiki maintenance aborted" >&2
  exit 1
fi
...
WORKTREE_WIKI_FILES=$(cd "$WORKTREE_DIR" && git diff --name-only HEAD \
  ...
  | grep -v 'gemini-commit-message\.txt' || true)
