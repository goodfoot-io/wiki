...
# Wiki Gap Detection (Codex)
#
# Scans recent commits for source files that have no wiki fragment link coverage.
# For each uncovered file that passes an inclusion gate, Codex creates or expands
...
LOG_FILE="$REPO_ROOT/$LOG_DIR/codex-wiki-gap-detection.log"
...
COMMIT_MSG_FILE="$WORKTREE_DIR/codex-commit-message.txt"
...
echo "Starting Codex Wiki Gap Detection..."

PROMPT="## Wiki Gap Detection
...
5. Write commit message to \`$COMMIT_MSG_FILE\`

Format: wiki: document [summary]"

if ! (cd "$WORKTREE_DIR" && timeout 900 codex --approval-mode full-auto "$PROMPT"); then
  echo "Codex exited with error — gap detection aborted" >&2
  exit 1
fi
...
} | grep '\.md$' | grep -v 'codex-commit-message\.txt' | sort -u | while IFS= read -r f; do
  [ -f "$f" ] || continue
  if awk '
