...
# Wiki Gap Detection (Claude)
#
# Scans recent commits for source files that have no wiki fragment link coverage.
# For each uncovered file that passes an inclusion gate, Claude creates or expands
...
LOG_FILE="$REPO_ROOT/$LOG_DIR/claude-wiki-gap-detection.log"
...
echo "Starting Claude Wiki Gap Detection..."
...
if ! (cd "$WORKTREE_DIR" && timeout 900 claude --print --max-turns 30 "$PROMPT"); then
  echo "Claude exited with error — gap detection aborted" >&2
  exit 1
fi
