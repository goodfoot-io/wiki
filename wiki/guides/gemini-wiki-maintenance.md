...
The repository runs an automated wiki maintenance pass after every commit on the main workspace. [`scripts/gemini-wiki-maintenence.sh`](wiki/guides/scripts/gemini-wiki-maintenence.sh#L1-L307) invokes Gemini CLI to inspect stale fragment links, update prose, re-pin links, and commit the result — all without human intervention.

## When It Runs

The [post-commit hook](wiki/guides/.githooks/post-commit#L16-L20) fires the script in a background process (`&`) after every commit. It only runs on the `main` branch from `/workspace` (the main checkout) — guards on `git rev-parse --show-toplevel` and `--abbrev-ref HEAD` prevent it from firing inside card worktrees or on feature branches, which would cause concurrent, conflicting runs or unnecessary updates.
...
2. **Runs Gemini** inside the worktree with an explicit admin-level policy file and a structured multi-step prompt that enforces [**fragment link discipline**](wiki/guides/scripts/gemini-wiki-maintenence.sh#L183-L193). Gemini's home directory is isolated (`$ISOLATED_HOME`) and its environment is strictly controlled, though `PATH` is forwarded so the `wiki` binary and other essential tools remain available. It uses separate credentials and cannot write outside the worktree.
...
- [`scripts/gemini-wiki-maintenence.sh`](wiki/guides/scripts/gemini-wiki-maintenence.sh#L1-L307)
- [`post-commit hook`](wiki/guides/.githooks/post-commit#L16-L20)