...
The [check command](/packages/cli/src/commands/check.rs#L28-L29) runs a full validation pass: [frontmatter parsing](/packages/cli/src/frontmatter.rs#L55-L55), title/alias collision detection, wikilink resolution, and fragment link verification (file existence and line range bounds at the pinned SHA). With `--fix`, unpinned fragment links are pinned automatically rather than reported as errors — already-pinned links are never touched.
