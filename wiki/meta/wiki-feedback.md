...
- `wiki check` accepts [glob patterns as positional arguments](/packages/cli/src/main.rs#L117-L130), allowing focused validation of specific files (e.g. `wiki check "documentation/**/*.wiki.md"`). Default (no args) expands to `$WIKI_DIR/**/*.md`. This is useful for validating a single newly-created or converted page without scanning the whole repo, consistent with CLAUDE.md guidance to focus validation runs.
