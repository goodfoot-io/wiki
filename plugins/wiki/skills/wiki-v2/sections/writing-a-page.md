...
links-reviewed: 1                 # optional; required in practice for pages citing code
---
```
...
`wiki check` verifies the target exists and that any `#fragment` resolves to a real heading (`broken_anchor`). Slugs are GitHub-style ([slug rules](/packages/cli/src/headings.rs#L29-L46)): lowercase, spaces → single `-` each (not collapsed), punctuation dropped; duplicate headings get `-1`, `-2`, …. Fragments are percent-decoded before matching.
